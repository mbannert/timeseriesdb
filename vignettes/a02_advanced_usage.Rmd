...
```{r, eval=FALSE}
read_time_series(con, "ch.kof.barometer",
                 schema = "tsdb_test")
...
```{r,eval=FALSE}
tsl <- read_time_series_history(con,"ch.kof.barometer",
                                schema = "tsdb_test")
```
...
Similarily use [db_release_get_next()](../reference/db_release_get_next.html) to get the next upcoming
release. 
...
You can remove keys from a collection using [db_ts_remove_from_collection()](../reference/db_ts_remove_from_collection.html) and 
