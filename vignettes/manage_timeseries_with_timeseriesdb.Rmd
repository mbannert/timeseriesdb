...
This guide explains in applied fashion how to use {timeseriesdb} effectively in practice.
...
that you have installed R, PostgreSQL and the {timeseriesdb} package successfully and it 
also assumes that set up the timeseriesdb database backend using the setup routines that ship with the package. If you have troubling setting this up, please take a look at our 
[Installtion](Guide).
...
to the database. {timeseriesdb} ships with a convenience function that allows 
...
time series. _Crystall Ball's_ Swiss researchers have composed a nice set of time
