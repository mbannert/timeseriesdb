...
a particular threshold. {timeseriesdb} offer `db_dataset_trim_history()` and `db_ts_trim_history()` to facilitate this type of maintenance work. In production setups this functions might be used in batch mode / workflow automation. 
...
`db_release_update()` updates a release date and `db_release_cancel()` lets an admin 
...
`db_access_list_levels()` lists all access levels available for a schema. 
New levels are created easily. Note also that new levels can be made the default.

```{r,eval=FALSE}
db_access_create_level(con, 
...
set post creation as well using `db_access_set_default()`. To delete levels, 
run `db_access_delete_level()`

### Assigning Access

To assign an access level to a single time series use `db_ts_change_access()`, 
to assign an access level to an entire dataset, i.e., to all series in a set, use
`db_ts_change_access_dataset()`.
