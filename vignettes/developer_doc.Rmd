...
## Launch a Docker Based Dev Environment

{timeseriesdb} ships with a routine to start up a development environment running 
PostgreSQL 11. Simply run the *start_dev_docker.sh* located in the /inst folder of the {timeseriesdb} package. Make sure the shell script is executable by your user (type ```chmod +x start_dev_docker.sh``` if it's not). 

```
./start_dev_docker.sh
```

Given you have a docker environment such as docker Desktop up an running, the above
shell script will fire up a docker container running PostgreSQL 11 which contains a fully working {timeseriesdb} schema. Once your container is up and running you can reach out on port 1111 using R. 
