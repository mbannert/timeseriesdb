context("store_records once but with embargo (use case 4)")

# The release date of 2119 was chosen because in the future a release date
# before Sys.time() might become an errorable offense.
...
tsl_c <- list(
  ts1 = ts(100:400, 2019, frequency = 12),
  ts2 = ts(100:500, 2019, frequency = 4)
)
class(tsl_c) <- c("tslist", "list")
...
#     main_after_insert_3,
#   file = "tests/testdata/c4_store_records_data.RData"
# )
...
load("../testdata/c4_store_records_data.RData")
...
test_that("it removes stale releases", {
  skip_on_cran()
  skip_if_not(is_test_db_reachable())
  ...
  expect_equal(
    dbGetQuery(con, "SELECT count(*) FROM timeseries.timeseries_main")$count,
    2
  )
})

test_that("it removes stale releases with empty intervals", {
  skip_on_cran()
  skip_if_not(is_test_db_reachable())
