...
test_that("it calls through to store_time_series.ts_json", {
  store_ts_json <- mock()
  with_mock(
    store_time_series.ts_json = store_ts_json,
    {
      store_time_series("con", tsl)
      expect_called(store_ts_json, 1)
      expect_args(store_ts_json, 1, "con", to_ts_json(tsl), names(tsl), NULL, NULL, "timeseries_main", TRUE, "timeseries")
    }
  )
})
...
test_that("it calls through to store_time_series.ts_json", {
  ...
  with_mock(
    store_time_series.ts_json = store_ts_json,
    {
      ...
      expect_called(store_ts_json, 1)
      expect_args(store_ts_json, 1, "con", to_ts_json(dt), dt[, id], NULL, NULL, "timeseries_main", TRUE, "timeseries")
    }
  )
})
