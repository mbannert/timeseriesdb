...
# queries that update an all field
microbenchmark::microbenchmark(old = {
  dbExecute(con, "UPDATE md_after
                  SET meta_data = md_with_update.meta_data,
                  locale = md_with_update.locale
                  FROM md_with_update
                  WHERE md_with_update.ts_key = md_after.ts_key
                  AND md_with_update.locale = md_after.locale;")
  dbExecute(con, "
                  
                  ---
                  INSERT INTO md_after
                  SELECT md_with_update.ts_key,
                  md_with_update.locale,
                  md_with_update.meta_data
                  FROM md_with_update
                  LEFT OUTER JOIN md_after
                  ON md_after.ts_key = md_with_update.ts_key
                  AND md_after.locale = md_with_update.locale
                  WHERE md_after.ts_key IS NULL 
                  AND md_after.locale IS NULL;")
},
new = {
